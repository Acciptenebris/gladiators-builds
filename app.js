...
const PLAYSTYLES_DATA = [
    {id: "guards", name: "Стражи", description: "Гейские шары"}, {id: "crits", name: "Криты", description: "Сраные проки"},
    {id: "dodge", name: "Увороты", description: "Всегда качай меня"}, {id: "poison", name: "Яды", description: "В начале тащу, в конце ГХ"},
    {id: "shields", name: "Щиты", description: "Без проков бесполезен"}, {id: "healing", name: "Лечение", description: "Сильнее калечу чем лечу"},
    {id: "health", name: "Здоровье", description: "Я имба"}, {id: "vulnerability", name: "Уязвимость", description: "Файт две секунды"},
    {id: "rage", name: "Ярость", description: "Не ясно зачем я нужен"}, {id: "freeze", name: "Заморозка", description: "Удачи кастануть ульту"},
    {id: "chaos", name: "Хаос", description: "Непонятная тема вообще"}, {id: "ultimate", name: "Ульта", description: "Опять гитарист в бане"},
    {id: "attack", name: "Атака", description: "Моя бить"}
];

// ========== БАЗОВАЯ БАЗА ДАННЫХ БИЛДОВ ==========
const DEFAULT_BUILDS = [{
    "hero": "DRAGON KNIGHT", "requiredMustHave": [], "desiredMustHave": [], "requiredMustNotHave": ["healing", "rage", "chaos", "ultimate"],
    "desiredMustNotHave": [], "talents": "1 2 2", "tier": 2,
...
function loadBuilds() {
    try {
        ...
        const repairedBuilds = savedBuilds.filter(b => b && b.hero).map(build => ({
            ...
            talents: build.talents || '', comment: build.comment || '', tier: build.tier || 4, img: build.img || ''
        }));
...
function buildCardView(build) {
    ...
    el.innerHTML = `${imgHtml}<div class="build-content"><div class="build-header"><div class="hero-name">${build.hero}
        <span class="tier-badge tier-badge-${build.tier||4}" title="Тир билда">${build.tier||4}</span></div>
        <div class="efficiency-badge ${efficiencyClass}" title="Эффективность билда">${build.efficiency}%</div></div>
...
function renderBuildsList() {
    ...
    indexedBuilds.forEach(({build, originalIndex}) => {
        ...
        el.innerHTML = `<div class="build-main-info"><div class="build-hero-name">${imgHtml}${build.hero || '(без имени)'}
          <span class="tier-badge tier-badge-${build.tier||4}" title="Тир">${build.tier||4}</span></div>
...
function showBuildFormModal(build, title) {
    ...
    if (!modal) {
        ...
        modal.innerHTML = `<div class="modal-content"><div class="modal-header"><h3>Создание билда</h3>
            ...
            <div class="row-inputs"><div class="form-field"><label>Герой:</label>
                <select id="build-hero" required><option value="">Выберите героя</option>
                    ${HEROES_LIST.map(hero => `<option value="${hero}">${hero}</option>`).join('')}</select></div>
                ...
                <div class="form-field"><label>Тир:</label><select id="build-tier">
                    ...
                    <option value="3">3 - Норм</option><option value="4">4 - Так себе</option></select></div></div>
...
function showAddBuildModal() {
    ...
    showBuildFormModal({ hero: '', requiredMustHave: [], desiredMustHave: [], requiredMustNotHave: [], desiredMustNotHave: [], talents: '', comment: '', tier: 4, img: '' }, 'Создание нового билда');
}
